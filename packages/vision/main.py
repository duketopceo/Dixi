...
def health_check():
    return jsonify({
        ...
        'camera_available': unified_tracking_service.camera is not None if unified_tracking_service.is_tracking else None,
...
def get_projector_gesture():
    ...
    # Extract gesture from tracking data (prefer right hand, fallback to left)
    hand_data = None
    if tracking_data and tracking_data.get('hands'):
        hand_data = tracking_data['hands'].get('right') or tracking_data['hands'].get('left')
    
    if not hand_data or not hand_data.get('detected') or hand_data.get('gesture') == 'none':
        return jsonify({
            'type': 'none',
    ...
    # Build gesture object similar to old format
    gesture = {
        'type': hand_data.get('gesture', 'none'),
        'position': hand_data.get('position', {'x': 0, 'y': 0}),
